...
class TextMelDataset(torch.utils.data.Dataset):
    ...
    def get_mel(self, filepath):
        spec_filepath = filepath.replace(".wav", ".spec.pt")
        if os.path.exists(spec_filepath):
            mel = torch.load(spec_filepath)
        else:
            audio, sr = ta.load(filepath)
            if sr != self.sample_rate:
                resampler = ta.transforms.Resample(sr, self.sample_rate)
                audio = resampler(audio)
            mel_stft = ta.transforms.MelSpectrogram(
            ...
            mel = mel_stft(audio)
            mel = mel.clamp(min=1e-5).log().squeeze()
            torch.save(mel, spec_filepath)
        return mel
