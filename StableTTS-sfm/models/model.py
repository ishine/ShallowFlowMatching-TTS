...
class StableTTS(nn.Module):
    def __init__(self, n_vocab, mel_channels, hidden_channels, filter_channels, n_heads, n_enc_layers, n_dec_layers, kernel_size, p_dropout, gin_channels):
        ...
        self.spk_emb = torch.nn.Embedding(109, hidden_channels)
        torch.nn.init.normal_(self.spk_emb.weight, 0.0, hidden_channels**-0.5)
    ...
    def forward(self, x, x_lengths, y, y_lengths, c):
        ...
        c = self.spk_emb(c)
        x, align_x, x_mask = self.encoder(x, c, x_lengths)
        logw = self.dp(x, x_mask, c)
        ...
        with torch.no_grad():
            ...
            neg_cent1 = torch.sum(-0.5 * math.log(2 * math.pi)- torch.zeros_like(align_x), [1], keepdim=True)
